#This is code is developed using the BookInfo project
...
# mongoURL = 'mongodb://172.31.212.87:30050/admin' if (os.environ.get("MONGO_DB_URL") is None) else os.environ.get("MONGO_DB_URL")
mongoURL = 'mongodb://10.68.116.136:30050/admin' if (os.environ.get("MONGO_DB_URL") is None) else os.environ.get("MONGO_DB_URL")
...
def fetch_books(points):
    book = mongo.db.books
    print("book number:", points)
    s = book.find_one({"_id": int(points)})
    print(s)
    if s:
        output = str(s) + "(fetch2)"
    else:
        output = "No such book (fetch2)"
